    ...
    in
    {
      ...
      config = {
        ...
        programs.fish = lib.mkIf cfg.fish.enable {
          ...
          shellAbbrs = {
            nrs = lib.mkIf pkgs.stdenv.hostPlatform.isLinux "sudo nixos-rebuild switch --flake .#${host}";
            nrt = lib.mkIf pkgs.stdenv.hostPlatform.isLinux "sudo nixos-rebuild test --flake .#${host}";
            drs = lib.mkIf pkgs.stdenv.hostPlatform.isDarwin "sudo darwin-rebuild switch --flake .#${host}";
