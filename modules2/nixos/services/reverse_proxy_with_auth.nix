...
# users.
{
  flake.modules.nixos.reverse_proxy_with_auth =
    {
      config,
      lib,
      user,
    ...
    in
    {
      ...
      config = lib.mkIf cfg.enable (
        let
          ...
          ldapDn = builtins.concatStringsSep "," (
            builtins.map (part: "dc=${part}") (lib.strings.splitString "." cfg.domain)
          );
          mkRedirects =
            service:
          ...
          mkVirtualHosts =
            ...
            mapAttrs' (
              name: service:
              nameValuePair "${service.subdomain}.${cfg.domain}" {
